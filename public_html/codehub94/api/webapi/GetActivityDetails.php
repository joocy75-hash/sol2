	...
	if ($_SERVER['REQUEST_METHOD'] != 'GET') {
		if (isset($shonupost['bannerId']) && isset($shonupost['language']) && isset($shonupost['random']) && isset($shonupost['signature']) && isset($shonupost['timestamp'])) {
			...
			if($shonusign == $signature){
				...
				if ($sesnum == 1) {
    $bannerId = intval($bannerId); // Safe side int cast
    $getBanner = $conn->query("SELECT * FROM activity_banner_custom_data WHERE bannerId = $bannerId AND status = 1 LIMIT 1");

    if ($getBanner->num_rows > 0) {
        $bannerData = $getBanner->fetch_assoc();

        $data['title'] = $bannerData['title'];
        $data['img'] = $bannerData['img'];
        $data['coverUrl'] = $bannerData['coverUrl'];
        $data['jumpType'] = intval($bannerData['jumpType']);
    }
