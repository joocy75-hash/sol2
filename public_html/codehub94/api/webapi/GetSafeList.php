...
if ($allow_origin) {
	...
	if ($_SERVER['REQUEST_METHOD'] != 'GET') {
		if (isset($shonupost['language']) && isset($shonupost['random']) && isset($shonupost['signature'])) {  // Removed timestamp condition
			$language = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['language']));
			$random = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['random']));
			$signature = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['signature']));
			$shonustr = '{"language":"'.$language.'","random":"'.$random.'"}';  // Fixed JSON formatting
			...
			if($signature){
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){					
						...
						$query = "SELECT id, user_id, motta, type, dayShareRate, orderNum, safeEarnings, earnings, created_at 
								  FROM safe_rec 
								  WHERE user_id = '$userId'ORDER BY id DESC LIMIT 5";
