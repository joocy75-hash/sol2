...
if ($allow_origin) {
	...
	if ($_SERVER['REQUEST_METHOD'] != 'GET') {
		if (isset($shonupost['language']) && isset($shonupost['random']) && isset($shonupost['signature'])) {
			...
			$shonustr = '{"language":"'.$language.'","random":"'.$random.'"}';  // Remove timestamp
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
                       $userId = $data_auth['payload']['id'];
                        $query = "SELECT safe,safeearn,safetoday FROM shonu_kaichila WHERE balakedara = $userId";
                        $result = mysqli_query($conn, $query);
                        $row = mysqli_fetch_assoc($result);

                 					
						$res['data'] = [
                            'balance' => (int)$row['safe'],
                            'safeEarnings' => $row['safeearn'],
                            'dayShareRate' => 0.00300
                        ];
                        $res['code'] = 0;
						$res['msg'] = 'Succeed';
						$res['msgCode'] = 0;
