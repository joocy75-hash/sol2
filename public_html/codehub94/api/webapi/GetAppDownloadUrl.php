...
if ($allow_origin) {
	...
	$query = "SELECT app_download FROM web_setting LIMIT 1";
	$result = mysqli_query($conn, $query);
	$row = mysqli_fetch_assoc($result);
	$app_download = $row['app_download'] ?? ''; // Default empty if no value found
	...
	if ($_SERVER['REQUEST_METHOD'] != 'GET') {		
		if (isset($shonupost['language']) && isset($shonupost['random']) && isset($shonupost['signature']) && isset($shonupost['timestamp'])) {
			...
			if($shonusign == $signature){				
				$data['androidUrl'] = $app_download ?: 'https://jashnwin.com/apk/JashnWin.apk'; // Fallback default link
				$data['iosUrl'] = '';
