...
if ($allow_origin) {
	...
	if ($_SERVER['REQUEST_METHOD'] != 'GET') {
		if (isset($shonupost['language']) && isset($shonupost['random']) && isset($shonupost['signature']) && isset($shonupost['timestamp'])) {
			$nikname = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['nikeName']));
          	$language = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['language']));
			$random = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['random']));
			$signature = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['signature']));
          	$nikname = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['nikeName']));
			$shonustr = '{"language":'.$language.',"nikeName":'.$nikname.', "random":"'.$random.'" }';
			...
			if($shonusign != $signature){
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
                    $sql = "UPDATE shonu_subjects SET codechorkamukala = '$nikname' WHERE id = $userId";
                    $conn->query($sql);
