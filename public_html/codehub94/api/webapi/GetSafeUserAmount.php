			...
			if($signature){
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
                        ...
                        $query = "SELECT safe, safeearn, motta FROM shonu_kaichila WHERE balakedara = $userId";
                        $result = mysqli_query($conn, $query);
                        $row = mysqli_fetch_assoc($result);

                        if (!$row) {
                            $res['code'] = 4;
                            $res['msg'] = 'No records found for this user';
                            $res['msgCode'] = 2;
                        ...
                        $res['data']['amountOfCode'] = 0;			
						$res['data']['balance'] = (float) $row['motta']; // Ensure balance is properly fetched
                        $res['code'] = 0;
						$res['msg'] = 'Succeed';
						$res['msgCode'] = 0;
